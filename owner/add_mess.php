...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = mysqli_prepare($conn, "INSERT INTO mess (owner_id, name, type, veg_type, location_area, address, latitude, longitude, description, 
                                                         cover_image, price_per_month, tags, facilities, owner_name, owner_contact, budget_friendly)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
        $ownerId = $_SESSION['owner_id'];
        $ownerName = $_SESSION['owner_name'];
        mysqli_stmt_bind_param($stmt, 'isssssddsisssssi',
